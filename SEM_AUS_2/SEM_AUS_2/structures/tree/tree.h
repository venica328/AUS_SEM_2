...
namespace structures
{
	...
	inline TreeNode<T>* TreeNode<T>::getBrother(int brothersOrder)
	{
		if (isRoot())
		{
			throw std::logic_error("Brother does not exist!");
		}
		else
		{
	...
	{
		//pri pouziti nejakej nasej vztvorenej struktury z frontov by to bola metoda peek()
	...
	{
		std::queue<TreeNode<T>*>* levelQueue = new std::queue<TreeNode<T>*>();
		levelQueue->push(current);

		while (levelQueue->empty() == false) {
			TreeNode<T>* levelNode = levelQueue->pop();
			if (levelNode != nullptr) {
				path_->push(levelNode);
				for (int i = 0; i < current->degree(); i++) {
					levelQueue->push(current->getSon(i));
				}
			}
		}
		delete levelQueue;
